...
import sounds
import ASCII
from lib import chooseLanguage, get_key, s, l
...
stick = Weapon(s(l,"wooden_stick"), 1, 1, 1, 0.2, 5, None, None, sounds.sticksound)
...
print(f"{s(l,'you_defeated')} {worm.name}!")
time.sleep(5)
...
print(ASCII.drawtrophy)
arcade.play_sound(sounds.victorysound)
print(f"{s(l,'you_defeated')} {fatWorm.name}!")
time.sleep(5)
...
while len(player.spells) == 0:
    ...
    choice = get_key()
    match choice:
        case 1: player.spells.append(FireSpell(s(l,"fireball"), 2, 4, 3, 0.5, 3))
        case 2: player.spells.append(IceSpell(s(l,"iceshard"), 1, 2, 3, 0.5, 2))
        case _: pass
...
print(ASCII.drawtrophy)
arcade.play_sound(sounds.victorysound)
print(f"{s(l,'you_defeated')} {wormKing.name}!")
time.sleep(5)
